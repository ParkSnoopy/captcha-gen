...
lazy_static! {
    static ref TOTAL: RwLock<usize> = RwLock::new(
        SOURCE.len().pow(LENGTH as u32) as usize
    );

    static ref CURRENT: Arc<RwLock<usize>> = Arc::new(RwLock::new(
        0
    ));
    ...
    static ref PRINT_EVERY: Arc<RwLock<usize>> = Arc::new(RwLock::new(
        min(
            10000,
            *TOTAL.read().unwrap() / 100,
        )
    ));
}
        ...
        .map(|target| {
            ...
            *CURRENT.write().unwrap() += 1;
            if *CURRENT.read().unwrap() % *PRINT_EVERY.read().unwrap() == 0 {
                let prg = (100_f64) * (*CURRENT.read().unwrap() as f64) / (*TOTAL.read().unwrap() as f64);
                println!("  - Progress: `{prg:.02?}`% ({}/{})", *CURRENT.read().unwrap(), *TOTAL.read().unwrap());
            }
        })
        .collect();
...
fn build(prefix: String, is_root: bool) -> Either<String, Vec<String>> {
    ...
    let mut results: Vec<String> = results.into_inner().expect("Failed to strip `RwLock` from `results`");
    ...
    if is_root {
        ...
        // Overwrite `TOTAL`
        println!("  - Target is built: ( Expected `{}`, After Dropout `{}` )", TOTAL.read().unwrap(), results.len());
